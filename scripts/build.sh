...
BUILD_DEB_CMD='GIT_VERSION=$(git rev-parse --short HEAD 2>/dev/null); VERSION=$(if [ -n "$GIT_VERSION" ]; then echo "0~g$GIT_VERSION"; else echo "0~$(date +%Y%m%d%H%M%S)"; fi); dch --create --package '"$PACKAGE_SRC_NAME"' -v ${VERSION} --distribution noble-porting --force-distribution "Bianbu Test"; DEB_BUILD_OPTIONS=nocheck dpkg-buildpackage -us -uc -b -ariscv64 -d'
